import axios from 'axios';
import { URL_DOCTORES } from '../../Constants/endpoints';

/**
 * @function agregarDoctor
 * @description Realiza una solicitud POST para agregar un nuevo doctor a la base de datos.
 * @param {object} doctor - El objeto del doctor a agregar.
 * @param {function} onComplete - La función de devolución de llamada que se ejecuta al completarse la solicitud.
 */
export const agregarDoctor = async (doctor, onComplete) => {
  try {
    // Realiza la solicitud POST a la URL de la API de doctores.
    const response = await axios.post(URL_DOCTORES, doctor);
    // Si la solicitud es exitosa, muestra una alerta y ejecuta la devolución de llamada.
    if (response) {
      alert('Doctor agregado con éxito');
      onComplete();
    }
  } catch (err) {
    // Si hay un error, lo muestra en la consola y en una alerta.
    console.error('Error al agregar doctor:', err);
    alert('Hubo un error al agregar el doctor.');
  }
};